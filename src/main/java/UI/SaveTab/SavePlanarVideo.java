...
public class SavePlanarVideo extends JFileChooser {
    ...
    public void save(){
        ...
        if(Home.getVideoProcessor().getPlanarCorrectionVid()==null){
            Object[] options = {"Ok"};
            JOptionPane.showOptionDialog(Home.getInterframe(), "Planar motion correction was not applied to video. " +
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            if(fileWithExt.exists() && !fileToSave.isDirectory()) {
                ...
                if (x ==1) {
                    ...
                    fileWithExt.delete();
                    //Create a new one
                    saveError = Home.getVideoProcessor().savePlanarCorrectionVid(fileWithExt.getPath());
                }else{
