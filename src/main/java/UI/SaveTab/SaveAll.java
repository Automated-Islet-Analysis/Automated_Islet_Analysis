...
public class SaveAll extends JFileChooser {
    ...
    public void save(){
        ...
        if(!Home.isMeanIntensityMeasured()){
            Object[] options = {"Ok"};
            JOptionPane.showOptionDialog(Home.getInterframe(), "Please measure the mean intensity of the ROIs first with the Measure intensity button in Data > Results!",
                    "Warning",
    ...
    public void callSavers(File directory){
        ...
        if(Home.getVideoProcessor().getPlanarCorrectionVid()!=null)
            saveError1=Home.getVideoProcessor().savePlanarCorrectionVid(directory.getPath()+"/planar_video_corrected.tif");
        else saveError1=SaveError.SAVE_SUCCESS;
        if(Home.getVideoProcessor().getDepthCorrectionVid()!=null)
            saveError2 = Home.getVideoProcessor().saveDepthCorrectionVid(directory.getPath() + "/depth_video_corrected.tif");
        else saveError2 = SaveError.SAVE_SUCCESS;
        saveError3=Home.getVideoProcessor().saveRoiImage(directory.getPath()+"/ROIs.jpg");
        ...
        saveError4=Home.getVideoProcessor().saveCellsMeanIntensity(MIFolder.getAbsolutePath());
        saveError5=Home.getVideoProcessor().saveSummary(saveDataFolder.getPath()+"/data_summary.csv");
