...
public class SaveData extends JFileChooser {
    ...
    public void save(){
        ...
        if(!Home.isMeanIntensityMeasured()){
            Object[] options = {"Ok"};
            JOptionPane.showOptionDialog(Home.getInterframe(), "Please measure the mean intensity of the ROIs" +
            ...
            return;
        }else if(Home.getVideoProcessor().getVideo().getCells().size()==0){
            Object[] options = {"Ok"};
            JOptionPane.showOptionDialog(Home.getInterframe(), "No regions of interest present, please add them.",
                    "Warning",
                    JOptionPane.OK_OPTION, JOptionPane.ERROR_MESSAGE, null, options, options[0]);
            return;
        }
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            if(!folder.isDirectory()) {
                Object[] options = {"Ok"};
                JOptionPane.showOptionDialog(Home.getInterframe(), "Please input an existing directory name!",
                        "Warning",
                        JOptionPane.OK_OPTION, JOptionPane.ERROR_MESSAGE, null, options, options[0]);
                new SaveData().save();
                ...
                // Save files
                saveError1 = Home.getVideoProcessor().saveCellsMeanIntensity(MIFolder.getAbsolutePath());

                if (summaryFile.exists())
                    summaryFile.delete();
                saveError2 = Home.getVideoProcessor().saveSummary(summaryFile.getPath());
