...
public class DynamicPanel extends JPanel {
    ...
    protected BufferedImage resizeImage(BufferedImage imgIn, JPanel panel){
        int w,h;
        double ratio = (double)imgIn.getWidth()/(double)imgIn.getHeight();
        double frameRatio = ((double) panel.getWidth()-marginHorizontal) / ((double)panel.getHeight()-marginVertical);

        // Adapt to fit in display based on width ratio
        if (frameRatio < ratio) {
            double scaling = (double) (panel.getWidth() - marginHorizontal) / (double) imgIn.getWidth();
        ...
        // Adapt to fit in display based on height ratio
            ...
            double scaling = (double) (panel.getHeight() - marginVertical) / (double) imgIn.getHeight();
            w = (int) Math.round(imgIn.getWidth() * scaling);
            h = (int) Math.round(imgIn.getHeight() * scaling);
            this.scalingOfImg=scaling;
        }
