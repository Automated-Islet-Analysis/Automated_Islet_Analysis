...
public class CLAHE implements PlugIn
{
    ...
    public final boolean setup(int blockRadius, int bins, float slope)
    {
...
//        if ( gd.wasCanceled() ) return false;
    ...
    public void run( String arg )
    {
        ...
        if ( !setup(127,256, (float) 3.00) ) return;
        ...
        for ( int y = 0; y < imp.getHeight(); ++y )
        {
            ...
            for ( int yi = yMin; yi < yMax; ++yi )
                for ( int xi = 0; xi < xMax0; ++xi )
                    ++hist[ round( src.get( xi, yi ) / 255.0f * bins ) ];

            for ( int x = 0; x < imp.getWidth(); ++x )
            {
                final int v = round( src.get( x, y ) / 255.0f * bins );
                ...
                if ( xMin > 0 )
                {
                    ...
                    for ( int yi = yMin; yi < yMax; ++yi )
                        --hist[ round( src.get( xMin1, yi ) / 255.0f * bins ) ];
                }
                ...
                if ( xMax < imp.getWidth() )
                {
                    ...
                    for ( int yi = yMin; yi < yMax; ++yi )
                        ++hist[ round( src.get( xMax1, yi ) / 255.0f * bins ) ];
                }
                ...
                dst.set( x, y, round( ( cdf - cdfMin ) / ( float )( cdfMax - cdfMin ) * 255.0f ) );
            }
            ...
            else if ( imp.getType() == ImagePlus.GRAY16 )
            {
                ...
                for ( int x = 0; x < imp.getWidth(); ++x )
                {
                    ...
                    ip.set( i, Math.max( 0, Math.min( 65535, round( a * ( v - min ) + min ) ) ) );
                }
            }
            ...
            else if ( imp.getType() == ImagePlus.COLOR_RGB )
            {
                for ( int x = 0; x < imp.getWidth(); ++x )
                {
                    ...
                    final float a = ( float )dst.get( i ) / src.get( i );
                    final int r = Math.max( 0, Math.min( 255, round( a * ( ( argb >> 16 ) & 0xff ) ) ) );
                    final int g = Math.max( 0, Math.min( 255, round( a * ( ( argb >> 8 ) & 0xff ) ) ) );
                    final int b = Math.max( 0, Math.min( 255, round( a * ( argb & 0xff ) ) ) );
                    ip.set( i, ( r << 16 ) | ( g << 8 ) | b );
                }
            }
            imp.updateAndDraw();
